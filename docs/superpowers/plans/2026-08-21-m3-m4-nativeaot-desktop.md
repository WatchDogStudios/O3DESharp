...
**`ReflectionInternalCalls` in the ABI.** ABI v1 covers the 47 pointers of `O3DE.InternalCalls` only. The ~20 reflection-dispatcher pointers are registered separately by `GenericDispatcher`, but neither `O3DE.InternalCalls` nor `O3DE.Reflection.ReflectionInternalCalls` are populated from the received `NativeImports` struct anywhere in this plan's code — that wiring is unbuilt and is required before a NativeAOT image can make any native call. Adding reflection-dispatcher support to the ABI is an ABI v2 change — which is what the version field exists for, and what the golden contract test will force to be done on both sides at once.
