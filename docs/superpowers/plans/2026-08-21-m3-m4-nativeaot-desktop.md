...
**`ReflectionInternalCalls` in the ABI.** ABI v1 covers the 47 pointers of `O3DE.InternalCalls` only. The ~20 reflection-dispatcher pointers are registered separately by `GenericDispatcher` and are **not** populated from the received `NativeImports` struct — `NativeImports` v1 has no fields for them at all, by design. Adding reflection-dispatcher support to the ABI is an ABI v2 change — which is what the version field exists for, and what the golden contract test will force to be done on both sides at once.
>
> **Update (post-plan):** the companion gap this entry originally also described — that `O3DE.InternalCalls`' 47 fields themselves were never wired from `NativeImports` either, so *no* native call worked under NativeAOT, not just reflection dispatch — has been closed. `Assets/Scripts/O3DE.Core/Interop/NativeImportsWiring.cs` (whole-file guarded under `O3DE_HOST_NATIVEAOT`, called from the generated `O3DESharp_GetManagedExports`) now cast-assigns each of the 47 `InternalCalls` fields from the matching `NativeImports` field, cross-checked against `InternalCalls.cs` by name and cast type by `Editor/Tests/test_native_imports_wiring.py`, with a real `dotnet build -p:O3DESharpHostMode=NativeAot` confirming every cast compiles. This was outside the plan's original 19 tasks; the `ReflectionInternalCalls`/ABI-v2 scope note above still stands unchanged.
